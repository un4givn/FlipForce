...
def store_snapshot_and_update_max(conn, pack_detail_data):
    with conn.cursor() as cur:
        ...
        series_tier = "Unknown"
        if pack_detail_data.get("slabPackCategory") and isinstance(
            pack_detail_data["slabPackCategory"], dict
        ):
        ...
        if cost is None and pack_detail_data.get("slabPackCategory"):
        ...
        cur.execute(
            ...
            INSERT INTO pack_series_metadata
                (series_id, name, tier, cost_cents, status)
            VALUES (%s, %s, %s, %s, %s)
...
def compute_newly_sold_cards_and_snapshot(conn, series_id, current_cards_with_tier):
    current_card_ids = set(
        card["id"] for card in current_cards_with_tier if "id" in card
    )

    with conn.cursor() as cur:
        cur.execute(
            "SELECT * FROM pack_card_snapshots WHERE series_id = %s;", (series_id,)
        )
        ...
        if column_names:
            for row in prev_snapshots:
                ...
                if "card_id" in card_dict:
                    ...
                    print(
                        f"[ERROR] 'card_id' not found in columns of "
                        f"pack_card_snapshots. Columns: {column_names} "
                        f"for row: {row}"
                    )
        ...
        cur.execute(
            "DELETE FROM pack_card_snapshots WHERE series_id = %s;", (series_id,)
        )
        ...
        if current_cards_with_tier:
            for card_api_data in current_cards_with_tier:
                if "id" not in card_api_data:
                    print(
                        f"[ERROR] Card data missing 'id': "
                        f"{card_api_data.get('playerName', 'Unknown Player')}"
                    )
                    continue
            ...
            if snapshot_insert_values:
                print(
                    f"[DEBUG] Inserting snapshot for series {series_id} "
                    f"with {len(snapshot_insert_values)} cards."
                )
                cur.executemany(
                    """
                ...
                print(
                    f"[DEBUG] current_cards_with_tier provided for series "
                    f"{series_id}, but no valid cards found to snapshot "
                    "after filtering."
                )
...
def store_current_pack_total_value(conn, series_id, total_value_cents):
    """
    Stores a snapshot of the total estimated value of all cards currently
    in the pack series.
    """
    try:
        with conn.cursor() as cur:
        ...
        conn.commit()
        print(
            f"[{datetime.now()}] Recorded total available pack value for "
            f"series {series_id}: {total_value_cents} cents."
        )
    except Exception as e:
        print(
        ...
        if conn:
            try:
                ...
                print(
                    f"[{datetime.now()}] ERROR during rollback for total pack value: "
                    f"{rb_e}"
                )
    ...
    try:
        ...
        while True:
            ...
            for target_pack in TARGET_PACKS_TO_TRACK:
                ...
                for category_from_api in all_categories_data.get("items", []):
                    if (
                        category_from_api.get("name", "").lower()
                        == target_cat_name_lower
                    ):
                        for series_in_category in category_from_api.get(
                            "slabPackSeries", []
                        ):
                            if (
                                series_in_category.get("name", "").lower()
                                == target_series_name_lower
                            ):
                                found_series_id_for_target = series_in_category.get(
                                    "id"
                                )
                                if found_series_id_for_target:
                                    discovered_series_to_process.append(
                                        {  # Line 457 fix: remove comment
                                            "id": found_series_id_for_target,
                                            "category_name_log": category_from_api.get(
                                                "name"
                                            ),
                                            "series_name_log": series_in_category.get(
                                                "name"
                                            ),
                                        }
                                    )
                                    print(
                                        f"[{datetime.now()}] Found Series ID: "
                                        f"{found_series_id_for_target} for target: "
                ...
                if not found_series_id_for_target:
                    print(
                        f"[{datetime.now()}] WARN: Could not find current series_id "
                        f"for target: {target_pack['category_name']} - "
                        f"{target_pack['series_name']}"
                    )

            if not discovered_series_to_process:
                print(
                    f"[{datetime.now()}] No series IDs found for targeted packs in "
                    "this cycle. Waiting 5 minutes..."
                )
                time.sleep(300)
                continue
            ...
            for series_info_to_process in discovered_series_to_process:
                ...
                if detailed_pack_data:
                    ...
                    if not api_series_id_in_detail:
                        print(
                            f"[{datetime.now()}] ERROR: 'id' not found in "
                            f"detailed_pack_data for discovered ID "
                            f"{current_series_id_from_discovery}. Skipping."
                        )
                        continue

                    if api_series_id_in_detail != current_series_id_from_discovery:
                        print(
                            f"[{datetime.now()}] WARN: ID mismatch! "
                            f"Discovered: {current_series_id_from_discovery}, "
                            f"Detail Endpoint: {api_series_id_in_detail} for "
                            f"{series_info_to_process['series_name_log']}. "
                            f"Using ID from detail endpoint: "
                            f"{api_series_id_in_detail}."
                        )
                    ...
                    if isinstance(slab_pack_tiers, list):
                        for tier_info_item in slab_pack_tiers:
                            if isinstance(tier_info_item, dict):
                                tier_name_from_slab = tier_info_item.get(
                                    "name", "Unknown Tier"
                                )
                                ...
                                if isinstance(cards_in_tier_list, list):
                                    for card_data_item in cards_in_tier_list:
                                        if isinstance(card_data_item, dict):
                                            card_api_data_with_tier = (
                                                card_data_item.copy()
                                            )
                                            card_api_data_with_tier["tier_name"] = (
                                                tier_name_from_slab
                                            )
                                            all_cards_for_this_series_with_tier.append(
                                                card_api_data_with_tier
                                            )
                                        else:  # Line 553 fix
                                            print(
                                                f"[{datetime.now()}] Card data in tier "
                                                f"'{tier_name_from_slab}' for series "
                                                f"{authoritative_series_id} is not a "
                                                f"dict: {card_data_item}"
                                            )
                                else:  # Line 560 fix
                                    print(
                                        f"[{datetime.now()}] 'cards' key in "
                                        f"tier_info for series "
                                        f"{authoritative_series_id}, tier "
                                        f"'{tier_name_from_slab}', is not a list: "
                                        f"{cards_in_tier_list}"
                                    )
                            else:
                                print(
                                    f"[{datetime.now()}] Tier data item for series "
                                    f"{authoritative_series_id} is not a dict: "
                                    f"{tier_info_item}"
                                )
                    else:
                        print(
                            f"[{datetime.now()}] WARN: 'slabPackTiers' not found or "
                            f"not a list in detailed_pack_data for series "
                            f"{authoritative_series_id}. Card-specific tiers "
                            "(Grail, Chase) cannot be extracted."
                        )
                    ...
                    for card_data in all_cards_for_this_series_with_tier:
                        ...
                        if isinstance(value, (int, float)):
                            ...
                            print(
                                f"[{datetime.now()}] WARN: 'estimatedValueCents' "
                                f"for a card in series {authoritative_series_id} "
                                f"is not a number: {value}. Skipping this value."
                            )
                        # If value is None, it's correctly ignored (adds 0
                        # implicitly if we started sum with 0)

                    if db_conn:
                        store_current_pack_total_value(
                            db_conn,
                            authoritative_series_id,
                            current_total_available_value_cents,
                        )

                    store_snapshot_and_update_max(db_conn, detailed_pack_data)
                    sold_count = compute_newly_sold_cards_and_snapshot(
                        db_conn,
                        authoritative_series_id,
                        all_cards_for_this_series_with_tier,
                    )
                    ...
                    total_value_log_cents = current_total_available_value_cents
                    total_value_log = (
                        f"{total_value_log_cents / 100.0:.2f}"
                        if isinstance(total_value_log_cents, (int, float))
                        else "N/A"
                    )
                    # Line 622 fix
                    print(
                        f"[{datetime.now()}] Pack processed: {pack_name_log} "
                        f"(Series ID: {authoritative_series_id}) | Cards in API: "
                        f"{len(all_cards_for_this_series_with_tier)} | "
                        f"Sold this run: {sold_count} | "
                        f"Total Value Now: ${total_value_log}"
                    )
                else:
                    print(
                        f"[{datetime.now()}] No detailed data fetched for series "
                        f"(Discovered ID: {current_series_id_from_discovery}). "
                        "Skipping."
                    )
                time.sleep(2)

            print(
                f"[{datetime.now()}] Completed processing all targeted series for "
                "this iteration. Waiting 15 minutes before next cycle..."
            )
        ...
        if db_conn:
            try:
                ...
                print(
                    f"[{datetime.now()}] Error closing DB connection during "
                    f"exception handling: {db_close_e}"
                )
            db_conn = None
            print(
                f"[{datetime.now()}] DB connection closed due to error. Tracker will "
                "attempt to re-establish on next cycle if error is "
                "recoverable, or exit if loop is broken."
            )
            time.sleep(60)
    finally:
        if db_conn:
...
if __name__ == "__main__":
    ...
    try:
        ...
        if temp_conn_for_schema:
            ...
            print(
                f"[{datetime.now()}] Failed to get DB connection for "
                "schema setup. Exiting tracker."
            )
            exit(1)
    except Exception as schema_err:
        print(
            f"[{datetime.now()}] Error during schema run: {schema_err}. "
            "Continuing to tracker, but DB might not be set up correctly."
        )
    finally:
        if temp_conn_for_schema:
