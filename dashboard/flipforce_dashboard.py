...
from sqlalchemy import create_engine  # Added for SQLAlchemy integration
...
def get_dashboard_db_engine():  # Modified for SQLAlchemy
...
def fetch_sold_cards_and_pack_metadata(db_engine):  # Parameter updated
    ...
    """
    df = pd.read_sql(query, db_engine)  # Uses SQLAlchemy engine
    if not df.empty:
...
def fetch_pack_total_value_data(db_engine):  # Parameter updated
    ...
    latest_df = pd.read_sql(
        query_latest_total_value, db_engine
    ...
    """
    min_max_df = pd.read_sql(
        query_min_max_total_value, db_engine
    )  # Uses SQLAlchemy engine
    return latest_df, min_max_df


def fetch_historical_value_trend_data(db_engine):  # Parameter updated
...
def fetch_ev_roi_data(db_engine):  # Parameter updated
    ...
    """
    latest_ev_roi_df = pd.read_sql(
        query_latest_ev_roi, db_engine
    )  # Uses SQLAlchemy engine
    ...
    """
    min_max_roi_df = pd.read_sql(query_min_max_roi, db_engine)  # Uses SQLAlchemy engine
    ...
    for _, row in unique_series_display_df.iterrows():
        ...
        if total_sold_for_this_series > 0:
            ...
            display_cols_for_sold_cards = [
                ...
                "card_tier",
                "overall",
                "insert_name",  # Added insert_name
                "sold_card_value_cents",
                "sold_at",
                "front_image",
            ]
            for _, card_event_row in series_specific_sold_events.head(
                10
            ).iterrows():  # Display top 10 recent
...
def load_all_dashboard_data(n_intervals):
    db_engine = None  # Changed variable name
    try:
        db_engine = get_dashboard_db_engine()  # Use the new engine function
        if not db_engine:
            error_message = (
                "Failed to create database engine. Please check configuration and logs."
            )
            return html.P(error_message, className="text-danger"), html.Div(
                error_message, className="text-danger"
            )

        df_sold_cards_and_meta = fetch_sold_cards_and_pack_metadata(
            db_engine
        )  # Pass engine
        df_latest_total_val, df_hist_min_max_total_val = fetch_pack_total_value_data(
            db_engine
        )  # Pass engine
        df_hist_val_trend_graph_data = fetch_historical_value_trend_data(
            db_engine
        )  # Pass engine
        df_latest_ev_roi, df_hist_min_max_roi = fetch_ev_roi_data(
            db_engine
        )  # Pass engine
        ...
        if not df_latest_total_val.empty:
            df_display_for_cards = pd.merge(
                df_display_for_cards, df_latest_total_val, on="series_id", how="left"
            )
        else:
        ...
        if not df_hist_min_max_roi.empty:
            df_display_for_cards = pd.merge(
                df_display_for_cards, df_hist_min_max_roi, on="series_id", how="left"
            )
        else:
        ...
        series_cards_ui = make_series_cards(
            df_display_for_cards, df_hist_val_trend_graph_data, df_sold_cards_and_meta
        )
        ...
        if db_engine:
            db_engine.dispose()  # Use dispose for SQLAlchemy engine
...
if __name__ == "__main__":
    if not all(
        ...
        # Attempt to create an engine to validate config early, but don't keep it open globally here
        test_engine = None
        try:
            ...
            if test_engine:
                ...
                # Potentially exit if essential, or let Dash try and fail within callbacks
        except Exception as e:
